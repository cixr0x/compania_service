...
describe('ProjectStakeholdersTotalService', () => {
  const findMany =
    jest.fn<(args: unknown) => Promise<Array<{ stakePercentage: string }>>>();
  const prisma = {
    projectStakeholder: {
      findMany,
    },
  ...
  it('returns valid when project stakeholder total is exactly 100', async () => {
    ...
    const service = new ProjectStakeholdersTotalService(
  ...
  it('throws BadRequestException when project stakeholder total is not exactly 100', async () => {
    ...
    await expect(service.validateTotal(10)).rejects.toBeInstanceOf(
      BadRequestException,
    );
  });
});
