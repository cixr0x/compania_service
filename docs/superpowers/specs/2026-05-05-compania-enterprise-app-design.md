...
- `is_active`: boolean flag stored as project metadata. The project create form defaults it to active.
...
- The desktop app shell should not render a persistent topbar/header. The left sider is the primary desktop navigation chrome; mobile uses only a small floating navigation trigger for the drawer.
- On mobile, use a controlled navigation drawer rather than the zero-width collapsed sider trigger so page content does not overlap navigation controls.
...
- Ant Design tables should use compact density and subtle striped body rows across entity lists, reports, import review tables, and inline detail tables.
...
- Project create/edit forms include an active flag plus unit cost. For projects whose selected model code is `ladrillo`, project cost is managed in a Project Cost Transactions detail section with a compact static table. Rows display date, description, and amount. Rows become editable only after selecting the row Edit action; editable rows expose Save and Cancel actions for that row. New transaction rows are added in edit mode, and project-level saving is blocked until all transaction row edits are saved or canceled. For non-`ladrillo` projects, the Project Cost Transactions section is hidden and its row validation/submission restrictions are not applied. The fixed production cost, administrative cost, cost adjustment, and adjustment description fields remain in the database for now but are not exposed in the form.
...
- Project create/edit forms include a Stakeholder Split detail section only for projects whose selected model code is `ladrillo`. The section loads stakeholders by name and uses a compact static table by default. Rows show stakeholder, stake percentage, and a read-only stake amount calculated as total project cost multiplied by stake percentage. Rows become editable only after selecting the row Edit action; editable rows expose Save and Cancel actions for that row. New stakeholder rows are added in edit mode, project-level saving is blocked until all stakeholder row edits are saved or canceled, complete rows must total exactly `100` when lines are present, and removing all rows saves an empty split. For non-`ladrillo` projects, the Stakeholder Split section is hidden and its row validation/submission restrictions are not applied.
